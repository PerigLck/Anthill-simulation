...
void onKeyPressed(char key, Environment * environment)
{
	...
	switch(key) {
			...
			case 'd':
				...
				(*(environment->getAllInstancesOf<food>().back())).supprimer();
				}
				break;
			case 'a':
				Anthill * f=new Anthill(environment,environment->randomPosition());
				for (int i=0;i<50;i++) {
//					new SillyAnt(environment,f->getCentre(),f);
					new Ant(environment,f->getPosition(),f,environment->randomPosition());
